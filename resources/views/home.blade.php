    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            function performCalculation() {
                ...
                if (!warningsDiv || !validationErrorsDiv || !helpContent || !resultsContent) {
        ...
        `;
                ...
                if (!seedingEl || !wellsEl || !areaEl || !mediaVolEl || !count1El || !viability1El || !
                    bufferEl || !suspensionVolEl) {
                ...
                const avgCount = validCounts.reduce((a, b) => a + b, 0) / validCounts.length;
                const avgViab = validViabilities.reduce((a, b) => a + b, 0) / validViabilities.length;
                ...
                if (warningsArr.length) {
                    warningsDiv.innerHTML = warningsArr.map((msg, index) => `
        ...
        `).join('');
                ...
                const volumePlatePerWellEl = document.getElementById('volume_plate_perwell_simple');
                const cellDensityFormattedEl = document.getElementById('cell_density_formatted');
                const requiredCellsFormattedEl = document.getElementById('required_cells_total_formatted');
                const cellsPerWellFormattedEl = document.getElementById('cells_per_well_formatted');
                ...
                if (volumePlatePerWellEl) volumePlatePerWellEl.textContent = (volPlatePerWell * 1000).toFixed(
                    0);
                if (cellDensityFormattedEl) cellDensityFormattedEl.innerHTML = formatExponential(cellDensity);
                if (requiredCellsFormattedEl) requiredCellsFormattedEl.innerHTML = formatExponential(
                    requiredCells);
                if (cellsPerWellFormattedEl) {
                    cellsPerWellFormattedEl.textContent = cellsPerWell.toLocaleString(undefined, {
            ...
            document.addEventListener('DOMContentLoaded', () => {
                // Recalculate functionality with error handling
                ...
                if (recalculateBtn) {
                    recalculateBtn.addEventListener('click', function(e) {
                        ...
                        try {
                            ...
                            alert(
                                'An error occurred during calculation. Please check your input values and try again.'
                            );
                        }
                    });
                } else {
                    console.warn('Recalculate button not found');
                }
